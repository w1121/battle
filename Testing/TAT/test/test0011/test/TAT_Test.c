...
static TAT_SRC_FILE_DEF tat_TAT_Test_test_SrcFiles[] = {
	{"D:\\Projects\\TAT\\test\\test0011\\mpr\\test.mpr", 1, 6, 13 }};
...
int TAT_Cmp_test_1_0_1 (ComplexData x, TAT_TEST_INSTANCE *id) {
	if(x.data[0]!=1) return 1;
	if(x.data[1]!=2) return 1;
	if(x.data[2]!=3) return 1;
	if(x.data[3]!=4) return 1;
	if(x.data[4]!=5) return 1;
	if(x.data[5]!=6) return 1;
	if(x.data[6]!=7) return 1;
	if(x.data[7]!=8) return 1;
	if(x.data[8]!=9) return 1;
	if(x.data[9]!=0) return 1;
	return 0;
}
