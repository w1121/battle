...
static TAT_SRC_FILE_DEF tat_TAT_Test_test_SrcFiles[] = {
	{"D:\\Projects\\TAT\\test\\test0008\\mpr\\test.mpr", 1, 6, 13 }};
...
ComplexData TAT_Ass_test_0_0 (TAT_TEST_INSTANCE *id) {
	ComplexData x;
	x.data=(char**)malloc(3*sizeof(char*));
	x.counter=3;
	x.data[0]=(char*)malloc(strlen("qwe")+1);
	strcpy(x.data[0],"qwe");
	x.data[1]=(char*)malloc(strlen("rty")+1);
	strcpy(x.data[1],"rty");
	x.data[2]=(char*)malloc(strlen("uiop")+1);
	strcpy(x.data[2],"uiop");
	return x;
}
int TAT_Cmp_test_1_0_1 (ComplexData x, TAT_TEST_INSTANCE *id) {
	if(x.counter!=3) return 1;
	if(strcmp(x.data[0],"qwe")) return 1;
	if(strcmp(x.data[1],"rty")) return 1;
	if(strcmp(x.data[2],"uiop")) return 1;
	return 0;
}
