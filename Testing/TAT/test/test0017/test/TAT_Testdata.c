...
int TAT_Dump_ComplexData(void *data, char *dest) {
	...
	while(cur) {
		strcat(dest,"(. ");
		strcat(dest,"(: ");
		for(j=0;j<cur->data.mIntPtr.counter;j++) {
			TAT_Dump_int(&(cur->data.mIntPtr.data[j]),dest);
			if(j<cur->data.mIntPtr.counter-1) strcat(dest,", ");
		}
		strcat(dest," :)");
		strcat(dest,", ");
		strcat(dest,"(: ");
		for(j=0;j<2;j++) {
			TAT_Dump_int(&(cur->data.mIntArr[j]),dest);
			if(j<1) strcat(dest,", ");
		}
		strcat(dest," :)");
		strcat(dest,", ");
		TAT_Dump_int(&(cur->data.mInt),dest);
		strcat(dest," .)");
		cur=cur->next;
		if(cur) strcat(dest,", ");
	}
...
ComplexData TAT_Copy_ComplexData(ComplexData src) {
	...
	scur=src;
	while(scur) {
		cur->data.mIntPtr.counter=scur->data.mIntPtr.counter;
		if(cur->data.mIntPtr.counter>0)
			cur->data.mIntPtr.data=(int*)malloc(cur->data.mIntPtr.counter*sizeof(int));
		else
			cur->data.mIntPtr.data=0;
		for(j=0;j<cur->data.mIntPtr.counter;j++) {
			cur->data.mIntPtr.data[j]=scur->data.mIntPtr.data[j];
		}
		for(j=0;j<2;j++) {
			cur->data.mIntArr[j]=scur->data.mIntArr[j];
		}
		cur->data.mInt=scur->data.mInt;
		scur=scur->next;
		if(scur)
...
int TAT_Compare_ComplexData(ComplexData x1, ComplexData x2) {
	...
	cur1=x1; cur2=x2;
	while(cur1 && cur2) {
		if(cur1->data.mIntPtr.counter!=cur2->data.mIntPtr.counter) return 1;
		for(j=0;j<cur1->data.mIntPtr.counter;j++) {
			if(cur1->data.mIntPtr.data[j]!=cur2->data.mIntPtr.data[j]) return 1;
		}
		for(j=0;j<2;j++) {
			if(cur1->data.mIntArr[j]!=cur2->data.mIntArr[j]) return 1;
		}
		if(cur1->data.mInt!=cur2->data.mInt) return 1;
		cur1=cur1->next;
		cur2=cur2->next;
