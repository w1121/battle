...
TAT_TEST_RESULT TAT_Test_sendMess(TAT_TEST_INSTANCE *id, TAT_EVENT_INSTANCE *event)
{
	...
	if (event->eventType == TAT_SEND_EV)
	{
		...
		case TAT_S_A_ID:
			{
				/*res = TAT_Test_A_Handler(((TAT_SIG_A *)event->eventData)->Param1);*/
				/* Use copy function to simplify memory allocation */
				x=TAT_Copy_Array3(((TAT_SIG_A *)event->eventData)->Param1);
				x.data[0].data[0].data[0]=(((TAT_SIG_A *)event->eventData)->Param1).data[1].data[1].data[1];
				x.data[0].data[0].data[1]=(((TAT_SIG_A *)event->eventData)->Param1).data[1].data[1].data[0];
				x.data[0].data[1].data[0]=(((TAT_SIG_A *)event->eventData)->Param1).data[1].data[0].data[1];
				x.data[0].data[1].data[1]=(((TAT_SIG_A *)event->eventData)->Param1).data[1].data[0].data[0];
				x.data[1].data[0].data[0]=(((TAT_SIG_A *)event->eventData)->Param1).data[0].data[1].data[1];
				x.data[1].data[0].data[1]=(((TAT_SIG_A *)event->eventData)->Param1).data[0].data[1].data[0];
				x.data[1].data[1].data[0]=(((TAT_SIG_A *)event->eventData)->Param1).data[0].data[0].data[1];
				x.data[1].data[1].data[1]=(((TAT_SIG_A *)event->eventData)->Param1).data[0].data[0].data[0];
				TAT_Test_Recv_B(x,TAT_I_IUT_ID,TAT_I_Test_ID);
				break;
			}
		default:
			{
...
*
*************************************************************************************/
int TAT_Test_Recv_B(Array3 Param1, int from, int to)
{
...
int main(int argc, char *argv[])
{
	...
	TI_ID = TAT_CreateTestInstance(256, 3, 4096, TAT_Test_resetSys,
 												 TAT_Test_sendMess,
 												 TAT_Test_clockF,
 												 TAT_Test_startCase,
 												 TAT_Test_finishCase);
