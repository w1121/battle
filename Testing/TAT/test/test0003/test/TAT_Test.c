...
static TAT_VARIABLE_DEF  tat_TAT_Test_test_Vardef[] = {
	{1, "res", sizeof(Array3), TAT_Dump_Array3}};
#define var_test_res (*(Array3*)id->vars[0])
static TAT_SRC_FILE_DEF tat_TAT_Test_test_SrcFiles[] = {
	{"D:\\Projects\\TAT\\test\\test0003\\mpr\\test.mpr", 1, 6, 13 }};
...
Array3 TAT_Ass_test_0_0 (TAT_TEST_INSTANCE *id) {
	Array3 x;
	x.data=(Array2*)malloc(2*sizeof(Array2));
	x.counter=2;
	x.data[0].data=(Array1*)malloc(2*sizeof(Array1));
	x.data[0].counter=2;
	x.data[0].data[0].data=(int*)malloc(2*sizeof(int));
	x.data[0].data[0].counter=2;
	x.data[0].data[0].data[0]=1;
	x.data[0].data[0].data[1]=2;
	x.data[0].data[1].data=(int*)malloc(2*sizeof(int));
	x.data[0].data[1].counter=2;
	x.data[0].data[1].data[0]=3;
	x.data[0].data[1].data[1]=4;
	x.data[1].data=(Array1*)malloc(2*sizeof(Array1));
	x.data[1].counter=2;
	x.data[1].data[0].data=(int*)malloc(2*sizeof(int));
	x.data[1].data[0].counter=2;
	x.data[1].data[0].data[0]=5;
	x.data[1].data[0].data[1]=6;
	x.data[1].data[1].data=(int*)malloc(2*sizeof(int));
	x.data[1].data[1].counter=2;
	x.data[1].data[1].data[0]=7;
	x.data[1].data[1].data[1]=8;
	return x;
}
int TAT_Cmp_test_1_0_1 (Array3 x, TAT_TEST_INSTANCE *id) {
	if(x.counter!=2) return 1;
	if(x.data[0].counter!=2) return 1;
	if(x.data[0].data[0].counter!=2) return 1;
	if(x.data[0].data[0].data[0]!=8) return 1;
	if(x.data[0].data[0].data[1]!=7) return 1;
	if(x.data[0].data[1].counter!=2) return 1;
	if(x.data[0].data[1].data[0]!=6) return 1;
	if(x.data[0].data[1].data[1]!=5) return 1;
	if(x.data[1].counter!=2) return 1;
	if(x.data[1].data[0].counter!=2) return 1;
	if(x.data[1].data[0].data[0]!=4) return 1;
	if(x.data[1].data[0].data[1]!=3) return 1;
	if(x.data[1].data[1].counter!=2) return 1;
	if(x.data[1].data[1].data[0]!=2) return 1;
	if(x.data[1].data[1].data[1]!=1) return 1;
	return 0;
}
int TAT_Cmp_test_3_0_1 (Array3 x, TAT_TEST_INSTANCE *id) {
	if(x.counter!=2) return 1;
	if(x.data[0].counter!=2) return 1;
	if(x.data[0].data[0].counter!=2) return 1;
	if(x.data[0].data[0].data[0]!=1) return 1;
	if(x.data[0].data[0].data[1]!=2) return 1;
	if(x.data[0].data[1].counter!=2) return 1;
	if(x.data[0].data[1].data[0]!=3) return 1;
	if(x.data[0].data[1].data[1]!=4) return 1;
	if(x.data[1].counter!=2) return 1;
	if(x.data[1].data[0].counter!=2) return 1;
	if(x.data[1].data[0].data[0]!=5) return 1;
	if(x.data[1].data[0].data[1]!=6) return 1;
	if(x.data[1].data[1].counter!=2) return 1;
	if(x.data[1].data[1].data[0]!=7) return 1;
	if(x.data[1].data[1].data[1]!=8) return 1;
	return 0;
}
	...
	if (id->queue.length>0) {
		...
		if ((_ev->eventTime.timeCalc.minTime-id->lastEvent>=0)) {
			if ((_ev->eventID == TAT_S_B_ID)&&
				...
				id->lastEvent = id->clock;
				TAT_Free_Array3(&(var_test_res));
				var_test_res=TAT_Copy_Array3(((TAT_SIG_B *)_ev->eventData)->Param1);
				if (id->dumpEvent(id, _ev, &tat_TAT_Test_test_1_event)!=TAT_TEST_CONTINUE) return TAT_TEST_CRITICAL;
				TAT_Free_Array3(&(((TAT_SIG_B *)_ev->eventData)->Param1));
				id->popQueue(id);
				id->nextState=2;
				return TAT_TEST_CONTINUE;
			}
...
static TAT_TEST_RESULT tat_test_state2(TAT_TEST_INSTANCE *id)
{
	TAT_EVENT_INSTANCE __ev;
		if ((id->clock-id->lastEvent>=0)) {
			...
			if (TAT_Send_A(id, &__ev, &tat_TAT_Test_test_2_event, TAT_Copy_Array3(var_test_res))!=TAT_TEST_CONTINUE) return TAT_TEST_CRITICAL;
				id->lastEvent = id->clock;
			id->nextState=3;
		}
	return TAT_TEST_FINISHED;
