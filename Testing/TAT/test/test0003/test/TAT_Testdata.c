...
int TAT_Dump_Array1(void *data, char *dest) {
	int i,j;
	strcat(dest,"(: ");
	for(i=0;i<((Array1 *)data)->counter;i++) {
		TAT_Dump_int(&(((Array1 *)data)->data[i]),dest);
		if(i<((Array1 *)data)->counter-1) strcat(dest,", ");
	}
	strcat(dest," :)");
	return 1;
}
...
int TAT_Dump_Array2(void *data, char *dest) {
	int i,j;
	strcat(dest,"(: ");
	for(i=0;i<((Array2 *)data)->counter;i++) {
		TAT_Dump_Array1(&(((Array2 *)data)->data[i]),dest);
		if(i<((Array2 *)data)->counter-1) strcat(dest,", ");
	}
	strcat(dest," :)");
	return 1;
}
...
int TAT_Dump_Array3(void *data, char *dest) {
	int i,j;
	strcat(dest,"(: ");
	for(i=0;i<((Array3 *)data)->counter;i++) {
		TAT_Dump_Array2(&(((Array3 *)data)->data[i]),dest);
		if(i<((Array3 *)data)->counter-1) strcat(dest,", ");
	}
	strcat(dest," :)");
	return 1;
}
...
Array1 TAT_Copy_Array1(Array1 src) {
	...
	if(x.counter>0)
		x.data=(int*)malloc(x.counter*sizeof(int));
	else
		x.data=0;
	for(i=0;i<x.counter;i++) {
		x.data[i]=src.data[i];
...
Array2 TAT_Copy_Array2(Array2 src) {
	...
	x.counter=src.counter;
	if(x.counter>0)
		x.data=(Array1*)malloc(x.counter*sizeof(Array1));
	else
		x.data=0;
	for(i=0;i<x.counter;i++) {
...
Array3 TAT_Copy_Array3(Array3 src) {
	...
	x.counter=src.counter;
	if(x.counter>0)
		x.data=(Array2*)malloc(x.counter*sizeof(Array2));
	else
		x.data=0;
	for(i=0;i<x.counter;i++) {
