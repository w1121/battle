...
static TAT_SRC_FILE_DEF tat_TAT_Test_test_SrcFiles[] = {
	{"D:\\Projects\\TAT\\test\\test0014\\mpr\\test.mpr", 1, 6, 13 }};
...
ComplexData TAT_Ass_test_0_0 (TAT_TEST_INSTANCE *id) {
	ComplexData x;
	x.mStrPtr.data=(char**)malloc(2*sizeof(char*));
	x.mStrPtr.counter=2;
	x.mStrPtr.data[0]=(char*)malloc(strlen("qwe")+1);
	strcpy(x.mStrPtr.data[0],"qwe");
	x.mStrPtr.data[1]=(char*)malloc(strlen("rty")+1);
	strcpy(x.mStrPtr.data[1],"rty");
	x.mStrArr[0]=(char*)malloc(strlen("ui")+1);
	strcpy(x.mStrArr[0],"ui");
	x.mStrArr[1]=(char*)malloc(strlen("op")+1);
	strcpy(x.mStrArr[1],"op");
	x.mStr=(char*)malloc(strlen("123")+1);
	strcpy(x.mStr,"123");
	return x;
}
int TAT_Cmp_test_1_0_1 (ComplexData x, TAT_TEST_INSTANCE *id) {
	if(x.mStrPtr.counter!=2) return 1;
	if(strcmp(x.mStrPtr.data[0],"qwe")) return 1;
	if(strcmp(x.mStrPtr.data[1],"rty")) return 1;
	if(strcmp(x.mStrArr[0],"ui")) return 1;
	if(strcmp(x.mStrArr[1],"op")) return 1;
	if(strcmp(x.mStr,"123")) return 1;
	return 0;
}
