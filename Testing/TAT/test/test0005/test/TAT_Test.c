...
static TAT_SRC_FILE_DEF tat_TAT_Test_test_SrcFiles[] = {
	{"D:\\Projects\\TAT\\test\\test0005\\mpr\\test.mpr", 1, 6, 17 }};
...
static TAT_INSTANCE_ID tat_I_TAT_Test_test_N4[]={TAT_I_Test_ID,TAT_I_IUT_ID};
static TAT_INSTANCE_ID tat_I_TAT_Test_test_N5[]={TAT_I_IUT_ID,TAT_I_Test_ID};
...
static TAT_SRC_REF_STRUCT tat_TAT_Test_test_4_SrcRef[] = {{0, 14}};
static TAT_SIG_A tat_TAT_Test_test_4_sigdata;
static TAT_EVENT_INSTANCE tat_TAT_Test_test_4_event = {TAT_SEND_EV, TAT_S_A_ID, &tat_TAT_Test_test_4_sigdata, {{0,0}, {0,0}, ""},2, 
(TAT_INSTANCE_ID *) &tat_I_TAT_Test_test_N4, "", 1, (TAT_SRC_REF_STRUCT *) tat_TAT_Test_test_4_SrcRef, 
TAT_VERDICT_PASS, -1, &tat_TAT_Test_test_MessageTable[4],
NULL};

static TAT_SRC_REF_STRUCT tat_TAT_Test_test_5_SrcRef[] = {{0, 16}};
static TAT_SIG_B tat_TAT_Test_test_5_sigdata;
static TAT_EVENT_INSTANCE tat_TAT_Test_test_5_event = {TAT_RECV_EV, TAT_S_B_ID, &tat_TAT_Test_test_5_sigdata, {{0,0}, {0,0}, ""},2, 
(TAT_INSTANCE_ID *) &tat_I_TAT_Test_test_N5, "", 1, (TAT_SRC_REF_STRUCT *) tat_TAT_Test_test_5_SrcRef, 
TAT_VERDICT_PASS, -1, &tat_TAT_Test_test_MessageTable[5],
NULL};

ComplexData TAT_Ass_test_0_0 (TAT_TEST_INSTANCE *id) {
	ComplexData x;
	x.data=(_ComplexData*)malloc(2*sizeof(_ComplexData));
	x.counter=2;
	x.data[0].mIntPtr.data=(int*)malloc(2*sizeof(int));
	x.data[0].mIntPtr.counter=2;
	x.data[0].mIntPtr.data[0]=1;
	x.data[0].mIntPtr.data[1]=2;
	x.data[0].mIntArr[0]=3;
	x.data[0].mIntArr[1]=4;
	x.data[0].mInt=5;
	x.data[1].mIntPtr.data=(int*)malloc(2*sizeof(int));
	x.data[1].mIntPtr.counter=2;
	x.data[1].mIntPtr.data[0]=6;
	x.data[1].mIntPtr.data[1]=7;
	x.data[1].mIntArr[0]=8;
	x.data[1].mIntArr[1]=9;
	x.data[1].mInt=0;
	return x;
}
int TAT_Cmp_test_1_0_1 (ComplexData x, TAT_TEST_INSTANCE *id) {
	if(x.counter!=2) return 1;
	if(x.data[0].mIntPtr.counter!=2) return 1;
	if(x.data[0].mIntPtr.data[0]!=1) return 1;
	if(x.data[0].mIntPtr.data[1]!=2) return 1;
	if(x.data[0].mIntArr[0]!=3) return 1;
	if(x.data[0].mIntArr[1]!=4) return 1;
	if(x.data[0].mInt!=5) return 1;
	if(x.data[1].mIntPtr.counter!=2) return 1;
	if(x.data[1].mIntPtr.data[0]!=6) return 1;
	if(x.data[1].mIntPtr.data[1]!=7) return 1;
	if(x.data[1].mIntArr[0]!=8) return 1;
	if(x.data[1].mIntArr[1]!=9) return 1;
	if(x.data[1].mInt!=0) return 1;
	return 0;
}
ComplexData TAT_Ass_test_4_0 (TAT_TEST_INSTANCE *id) {
	ComplexData x;
	x.data=0;
	x.counter=0;
	return x;
}
int TAT_Cmp_test_5_0_1 (ComplexData x, TAT_TEST_INSTANCE *id) {
	if(x.counter!=0) return 1;
	return 0;
}
...
static TAT_TEST_RESULT tat_test_state4(TAT_TEST_INSTANCE *id)
{
	TAT_EVENT_INSTANCE __ev;
		if ((id->clock-id->lastEvent>=0)) {
			memcpy(&__ev, &tat_TAT_Test_test_4_event, sizeof(__ev));
			if (TAT_Send_A(id, &__ev, &tat_TAT_Test_test_4_event, TAT_Ass_test_4_0(id))!=TAT_TEST_CONTINUE) return TAT_TEST_CRITICAL;
				id->lastEvent = id->clock;
			id->nextState=5;
		}
	return TAT_TEST_FINISHED;
}
static TAT_TEST_RESULT tat_test_state5(TAT_TEST_INSTANCE *id)
{
	TAT_EVENT_INSTANCE *_ev;
	if (id->queue.length>0) {
		_ev = id->getQueue(id);
		if ((_ev->eventTime.timeCalc.minTime-id->lastEvent>=0)) {
			if ((_ev->eventID == TAT_S_B_ID)&&
				((TAT_Cmp_test_5_0_1(((TAT_SIG_B *)_ev->eventData)->Param1,id) == 0))){
				id->lastEvent = id->clock;
				if (id->dumpEvent(id, _ev, &tat_TAT_Test_test_5_event)!=TAT_TEST_CONTINUE) return TAT_TEST_CRITICAL;
				TAT_Free_ComplexData(&(((TAT_SIG_B *)_ev->eventData)->Param1));
				id->popQueue(id);
				id->nextState=-1;
				return TAT_TEST_CONTINUE;
...
TAT_TEST_RESULT TAT_T_TAT_Test_test( TAT_TEST_INSTANCE* id )
{
	...
	tat_test_state0,tat_test_state1,tat_test_state2,tat_test_state3,tat_test_state4,
	tat_test_state5};
	TAT_TEST_RESULT res;
	int prevState;
	if( id == NULL )
