...
static TAT_SRC_FILE_DEF tat_TAT_Test_test_SrcFiles[] = {
	{"D:\\Projects\\TAT\\test\\test0013\\mpr\\test.mpr", 1, 6, 13 }};
...
ComplexData TAT_Ass_test_0_0 (TAT_TEST_INSTANCE *id) {
	ComplexData x;
	x.mIntPtr.data=(int*)malloc(2*sizeof(int));
	x.mIntPtr.counter=2;
	x.mIntPtr.data[0]=1;
	x.mIntPtr.data[1]=2;
	x.mIntArr[0]=3;
...
int TAT_Cmp_test_1_0_1 (ComplexData x, TAT_TEST_INSTANCE *id) {
	if(x.mIntPtr.counter!=2) return 1;
	if(x.mIntPtr.data[0]!=1) return 1;
	if(x.mIntPtr.data[1]!=2) return 1;
	if(x.mIntArr[0]!=3) return 1;
	if(x.mIntArr[1]!=4) return 1;
	if(x.mInt!=5) return 1;
	return 0;
}
